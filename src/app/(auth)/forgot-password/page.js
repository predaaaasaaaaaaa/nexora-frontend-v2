...
export default function ForgotPasswordPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (error) {
        setError(friendlyError(error, 'Failed to send reset email'))
        setLoading(false)
        return
      }
      setSuccess(true)
      ...
      <div style={{
        ...
        <div style={{ background: '#141414', border: '1px solid #222', borderRadius: 18, padding: 40, textAlign: 'center', maxWidth: 440, width: '100%' }}>
          ...
          <p style={{ fontSize: 14, color: '#888', lineHeight: 1.6, marginBottom: 24 }}>
            If an account exists for <strong style={{ color: '#F1F1F1' }}>{email}</strong>, we sent a password reset link. Click it to choose a new password.
          </p>
      ...
      <div style={{ width: '100%', maxWidth: 420, position: 'relative', zIndex: 2 }}>
        ...
        <div className="nx-auth-card" style={{ background: '#141414', border: '1px solid #222', borderRadius: 18, padding: 32, position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ textAlign: 'center', marginBottom: 28 }}>
            ...
            <p style={{ fontSize: 14, color: '#888' }}>Enter your email and we'll send you a reset link</p>
          </div>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
            <div>
              ...
              <div style={{ position: 'relative' }}>
                ...
                <input type="email" required placeholder="you@example.com" className="auth-input" value={email} onChange={e => setEmail(e.target.value)} style={inputStyle} />
              </div>
            </div>
          ...
          <div style={{ marginTop: 24, paddingTop: 20, borderTop: '1px solid #222', textAlign: 'center' }}>
            <Link href="/login" className="auth-link" style={{ fontSize: 14, color: '#FF0000', fontWeight: 600, textDecoration: 'none' }}>← Back to sign in</Link>
          </div>
        </div>
      </div>
    </div>
