...
const Toggle = ({ on, onToggle, c, accentColor }) => (
  <button onClick={onToggle} style={{
    ...
    background: on ? (accentColor || c.toggleOn) : c.toggleOff,
    position: 'relative', transition: 'background 0.25s ease', flexShrink: 0,
  }}>
    <div style={{
      width: 20, height: 20, borderRadius: '50%', background: '#fff',
      position: 'absolute', top: 3, left: on ? 25 : 3,
      transition: 'left 0.25s ease', boxShadow: '0 1px 4px rgba(0,0,0,0.2)',
    }}/>
  </button>
)
...
export default function SettingsPage() {
  ...
  async function loadYouTubeStatus() {
    ...
    catch (err) { console.error('Error loading YouTube status:', err); setYoutubeStatus({ connected: false }) }
  ...
  async function handleConnectYouTube() {
    ...
    catch (err) { console.error('Error connecting YouTube:', err); setYoutubeAction('error'); setTimeout(() => setYoutubeAction(null), 3000) }
  }
  ...
  async function handleDisconnectYouTube() {
    ...
    catch (err) { console.error('Error disconnecting YouTube:', err); setYoutubeAction('error'); setTimeout(() => setYoutubeAction(null), 3000) }
  }

  async function toggleNotifications() {
    if (!notifPrefs) return
    try {
      const updated = await updateNotificationPreferences({ ...notifPrefs, enabled: !notifPrefs.enabled })
      setNotifPrefs(updated.preferences)
      setSuccess(updated.preferences.enabled ? 'Notifications enabled' : 'Notifications paused')
      setTimeout(() => setSuccess(''), 3000)
    } catch (err) { console.error('Error toggling notifications:', err) }
  }
    ...
    <div style={{ maxWidth: 720 }}>
      ...
      {success && (
        <div style={{
          ...
          <I.Check />
          <span style={{ fontSize: 14, fontWeight: 500, color: c.green }}>{success}</span>
        </div>
      )}
          ...
          <div style={{ ...rowStyle(), opacity: 1 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
              ...
              <div>
                ...
                {youtubeLoading ? (
                  <div style={{ fontSize: 12, color: c.textDim }}>Checking...</div>
                ) : youtubeStatus?.connected ? (
                  <div style={{ fontSize: 12, fontWeight: 500, marginTop: 2, color: c.green, display: 'flex', alignItems: 'center', gap: 4 }}>
                    <I.Check /> Connected as {youtubeStatus.details?.platform_username}
                  </div>
                ) : (
            ...
            {youtubeLoading ? (
              ...
              <button className="disconnect-btn" onClick={handleDisconnectYouTube} disabled={youtubeAction === 'disconnecting'}
                style={{
                  padding: '7px 16px', borderRadius: 8, background: 'transparent', border: `1px solid ${c.border}`,
                  color: c.textSec, cursor: 'pointer', fontSize: 12, fontWeight: 600, fontFamily: 'inherit',
                  display: 'flex', alignItems: 'center', gap: 6, opacity: youtubeAction === 'disconnecting' ? 0.5 : 1,
                }}>
                {youtubeAction === 'disconnecting' ? <div style={{ width: 14, height: 14, border: `2px solid ${c.border}`, borderTopColor: c.red, borderRadius: '50%', animation: 'spin 0.8s linear infinite' }}/> : <I.Unlink />}
                Disconnect
              </button>
            ) : (
              <button onClick={handleConnectYouTube} disabled={youtubeAction === 'connecting'}
                style={{
                  ...
                  opacity: youtubeAction === 'connecting' ? 0.5 : 1,
                }}>
                {youtubeAction === 'connecting' ? <div style={{ width: 14, height: 14, border: '2px solid rgba(255,255,255,0.3)', borderTopColor: '#fff', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }}/> : <I.Link />}
                Connect
              </button>
            )}
          </div>
          ...
          {notifLoading ? (
            ...
            <div style={{ padding: '16px 24px 20px' }}>
              ...
              <Link href="/scheduler" style={{
                ...
                color: c.red, fontSize: 13, fontWeight: 500, textDecoration: 'none',
              }}>
                <I.Bell /> Set up reminders
              </Link>
            </div>
          )}
        </div>
      ...
      <div style={{
        ...
        <div style={{ ...rowStyle(true), borderBottomColor: c.dangerBorder }}>
          ...
          <button className="danger-btn" onClick={handleSignOut}
            style={{
              padding: '8px 20px', borderRadius: 8, background: 'transparent', border: `1px solid ${c.border}`,
              color: c.textSec, cursor: 'pointer', fontSize: 13, fontWeight: 600, fontFamily: 'inherit',
              display: 'flex', alignItems: 'center', gap: 6,
            }}>
            <I.LogOut /> Sign Out
          </button>
